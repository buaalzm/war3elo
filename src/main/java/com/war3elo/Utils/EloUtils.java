...
public class EloUtils {
    ...
    private final static int k = 32; // k 极限值，代表理论上最多可以赢一个玩家的得分和失分
     ...
     * @return			本轮结束后A、B玩家得分
     */
    public static ELOResult rating(int ra, int rb, float sa, boolean limit){

        // 统一由分数高的一方计算出积分变动情况
        if(ra < rb){
            ELOResult result = rating(rb, ra, 1.0f-sa, limit);
        ...
        BigDecimal ea = DONE.divide(DONE.add(new BigDecimal(Math.pow(10, new BigDecimal(rb - ra).divide(D400, 6, RoundingMode.HALF_UP).doubleValue()))), 6, RoundingMode.HALF_UP);
